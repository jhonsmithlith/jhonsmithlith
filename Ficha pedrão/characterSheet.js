const data = {
  name: 'Pedro/Pedrão',
  player: 'Gabriel',
  occupation: 'Mecanico/Engenehiro.',
  age: 37,
  sex: 'male',
  birthplace: 'Minas gerais/Belo horizonte',
  residence: 'São paulo',
...
$('#addWeaponForm').submit(function (event) {
  ...
  if ($('#weaponType').val() == 'Corte') {
    weaponType = 'C'
  } else if ($('#weaponType').val() == 'Impacto') {
    weaponType = 'I'
  } else if ($('#weaponType').val() == 'Perfurante') {
    weaponType = 'P'
  }else if ($('#weaponType').val() == 'Balistico') {
    weaponType = 'B'
  }
